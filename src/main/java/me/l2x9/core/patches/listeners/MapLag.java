...
public class MapLag implements PacketListener {
    ...
    public MapLag(final PatchManager manager) {
        this.manager = manager;
        try {
            (this.iconsF = PacketPlayOutMap.class.getDeclaredField("d")).setAccessible(true);
            (this.idF = PacketPlayOutMap.class.getDeclaredField("a")).setAccessible(true);
        }
        catch (Throwable t) {
            t.printStackTrace();
        }
